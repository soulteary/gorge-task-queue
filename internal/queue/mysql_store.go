...
func NewMySQLStore(dsn string, leaseDuration, retryWait int) (*MySQLStore, error) {
...
func (s *MySQLStore) Enqueue(ctx context.Context, req *EnqueueRequest) (*Task, error) {
...
func (s *MySQLStore) Lease(ctx context.Context, limit int, leaseOwner string) ([]*Task, error) {
	if limit <= 0 {
		limit = 1
	}
...
func (s *MySQLStore) Complete(ctx context.Context, taskID int64, duration int64) (*ArchivedTask, error) {
	return s.archiveTask(ctx, taskID, ResultSuccess, duration)
}

func (s *MySQLStore) Fail(ctx context.Context, req *FailRequest) error {
	if req.Permanent {
...
func (s *MySQLStore) Cancel(ctx context.Context, taskID int64) (*ArchivedTask, error) {
...
func (s *MySQLStore) Stats(ctx context.Context) (*QueueStats, error) {
	stats := &QueueStats{}
	now := time.Now().Unix()
...
func (s *MySQLStore) GetTask(ctx context.Context, taskID int64) (*Task, error) {
	t := &Task{}
	var leaseOwnerN, objectPHID, containerPHID sql.NullString
	var leaseExp, failureTime sql.NullInt64
...
func (s *MySQLStore) ListActive(ctx context.Context, limit, offset int) ([]*Task, error) {
	if limit <= 0 {
		limit = 100
	}
...
func (s *MySQLStore) archiveTask(ctx context.Context, taskID int64, result int, duration int64) (*ArchivedTask, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("begin tx: %w", err)
	}
